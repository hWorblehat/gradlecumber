...
import io.github.hWorblehat.gradlecumber.dsl.cucumberCheckResultsTaskName
import io.github.hWorblehat.gradlecumber.dsl.cucumberExecTaskName
import io.github.hWorblehat.gradlecumber.dsl.cucumberLifecycleTaskName
...
class GradlecumberPluginTest : FreeSpec({
	...
	GradlecumberPlugin::class.simpleName!! - {
		...
		"Adding rules to a suite splits apart execution and analysis" - {
			...
			val gradleMajorVersion = project.gradle.gradleVersion.split(".")[0].toInt()
			...
			"a ${CucumberCheckResults::class.simpleName} task is added with" - {
				...
				"a defined messages input file".config(
					enabled = gradleMajorVersion < 7
				) {
